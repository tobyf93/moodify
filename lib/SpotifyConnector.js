...
SpotifyConnector.prototype.getPlaylistTracks = function(playlistID) {
  ...
  return this.api.getPlaylistTracks(this.userID, playlistID)
    ...
    // TODO: Only doing first 20 due to URL length limits
    .then(() => {
      const playlistIDs = tracks.map((track) => track.id).slice(0, 20).join(',');
    ...
    // Add energy and valence to tracks
    .then((data) => {
